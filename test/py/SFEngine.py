...
class SFEngine():
        ...
        def callback(filename, start, feature):
            for freq in feature:
                ...
                for (origianl_filename, origianl_time) in self.freq_dict[freq]:
                    target_tuple.append((origianl_filename,
                                         (origianl_time - int(start)) / fs))
    ...
    def extract_feature(self, scaled, start, interval):
        ...
        length = len(dst) / 2
        normalized = abs(dst[:int(length - 1)])
        feature = [
            normalized[:50].argmax(), 50 + normalized[50:100].argmax(),
    ...
    def read_and_scale(self, filename):
        ...
        if data.ndim == 2:
            data = data.T[
                0]  # this is a two channel soundtrack, I get the first track
